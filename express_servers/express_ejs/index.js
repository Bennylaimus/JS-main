...
app.get('/random', (req, res) => {
    ...
    res.render('random.ejs', { number }); // or {num: number}
});

app.get('/random2', (req, res) => {
    const number = Math.floor(Math.random() * 10) + 1
    res.render('random2.ejs', { num: number }); // of {number}
});

app.get('/r/:subreddit', (req, res) => {
    const subtheme = req.params.subreddit
    res.render('subtheme.ejs', { subtheme }); // of {theme: subtheme}
});

app.get('/cats', (req, res) => {
    const cats = ['Monty', 'Belly', 'Pepo'];
    res.render('cats.ejs', { cats }); // or {allCats: cats}
});
